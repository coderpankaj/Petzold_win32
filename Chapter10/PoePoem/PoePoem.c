                    ...
                    PSTR szCmdLine, int iCmdShow)
{
     ...
     {
         // LoadStringW would fail for window98, hence this code is added.
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
     ...
     case WM_CREATE :
          ...
          /* LoadResource : Retrieves a handle that can be used to obtain a pointer to the first byte 
             ...
             FindResource : Determines the location of a resource with the specified type and name in 
                            the specified module.
                            
                            Note: LoadResource does not actually loads the resource into memory */
          hResource = LoadResource (hInst, 
                      FindResource (hInst, TEXT ("AnnabelLee"),
                                           TEXT ("TEXT"))) ;
          
          /* LockResource : Loads the resource into memory and returns a pointer to it. */
          if (hResource != NULL)
          {
              ...
              while (*pText != '\\' && *pText != '\0')
              {
                  ...
                  /* CharNext : Retrieves a pointer to the next character in a string. */
